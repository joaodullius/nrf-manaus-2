...
Só o instrutor precisa deste módulo — nenhum kit de aluno tem SIM NTN e não
há lab de aluno.

- **Python 3.12** com `pip install -r ntn/tools/requirements.txt`.
- **nRF Connect for Desktop › Programmer** — gravação do firmware de modem
  `mfw_nrf9151-ntn_1.0.1` (baixado à parte, licença Nordic; ver
  [`ntn/hex/README.md`](ntn/hex/README.md)).
- **`nrfutil install trace`** — só necessário para reconverter um `.bin` de
  trace novo em `.txt`; os traces já convertidos do repositório não
  precisam disso.
