/*
 * 09_dog_bark_detection — um zip do Edge AI Lab rodando na NPU Axon.
 *
 * Codigo do curso nrf-manaus-2 (modulo edge_ai/09_dog_bark_detection). O modelo em
 * src/nrf_edgeai_generated/ e a solucao pronta "sound-event-dog-bark" do Nordic
 ...
 * uma linha ">>>" a cada latido detectado.
 */
 ...
 * a cada ~30 ms, a probabilidade de haver latido na janela. Contamos quadros
 * brutos acima do limiar numa janela deslizante (mesmo padrao da wake word do
 * 07_ww_kws). Este modelo e "gatilho facil" — dispara em qualquer som
 * impulsivo — entao o limiar e alto e exige evidencia sustentada.
 * Valores calibrados na bancada do curso. */
#define RAW_THRESHOLD	0.90f
#define MIN_HITS	5
...
int main(void)
{
	...
	printk("Escutando. Toque um latido perto do microfone.\n\n");
	...
	while (true) {
		...
		if (cooldown > 0) {
			...
			history = 0;
			hits = 0;
			printk(">>> LATIDO (pico %d%%) — evento %u\n",
			       (int)(peak * 100.0f), events);
		}

		if (k_uptime_get() >= next_panel) {
			next_panel += PANEL_PERIOD_MS;
			printk("[latido] pico do ultimo segundo: %3d%%  (eventos: %u)\n",
			       (int)(peak * 100.0f), events);
			peak = 0.0f;
		}
	}
