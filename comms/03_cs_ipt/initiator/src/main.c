 ...
 *   Upstream: nrf/samples/bluetooth/channel_sounding/ipt_initiator/src/main.c
 *   Local   : C:/ncs/v3.4.0/nrf/samples/bluetooth/channel_sounding/ipt_initiator/src/main.c
 *   Copiado : 2026-09-05 — curso nrf-manaus-2, modulo comms/03_cs_ipt/initiator
 *
 * Para conferir se divergiu do SDK:
 *   diff <este arquivo> C:/ncs/v3.4.0/nrf/samples/bluetooth/channel_sounding/ipt_initiator/src/main.c
 *
 * DIVERGENCIA DO CURSO (duas):
 *   1. scan_init() e add_tag_address_filter(). O upstream filtra so pelo NOME
 *      "Nordic CS IPT Reflector", em modo OR — e todos os TAGs da sala tem
 *      esse nome. Aqui entra o filtro pelo endereco do TAG, digitado no
 *      terminal serial no boot (src/lab_tag_addr.c), e o modo passa a AND.
 ...
 * no primeiro que aparecesse. Aqui entra o filtro pelo endereco
 * digitado no terminal (lab_tag_addr_read) e o modo passa a AND (match_all = true): precisa ter
