...
Seis capturas reais do par nRF54LM20-DK (initiator, lab 5) ↔ nRF54L15-TAG (reflector,
...
As duas últimas são o **par do experimento de antenas**: mesma posição, mesma noite,
uma logo após a outra, trocando só `CONFIG_LAB_ANTENNA_PATHS` no initiator. A
conclusão está na seção "Duas antenas" do [README do lab](../README.md). O achado foi
confirmado em três rodadas alternadas (1 → 2 → 1 → 2 → 1 → 2 caminhos, ~100
procedures cada); o repo versiona uma rodada de cada, por tamanho. A terceira rodada
de **ambas** as configurações trouxe procedures ruins extras — alguma coisa mudou na
sala —, o que não alterou a comparação em MAD.
...
- **Antena única nas quatro primeiras.** O TAG tem duas antenas e o reflector as
  configura, mas o initiator usa um caminho por default. As duas capturas de 1,00 m
  existem justamente para medir o que muda com dois — e a resposta, em linha de visada,
  é: piora. Veja a seção "Duas antenas" do README do lab.
