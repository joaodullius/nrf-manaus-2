...
/* Masks of the buttons this module reports, in button-number order. */
 ...
 *
 * Long press is tracked for button 1 only, which is the only one that ever
 * carried a second meaning. The others report short presses.
 */
static void button_handler(uint32_t button_states, uint32_t has_changed)
{
	for (int i = 0; i < ARRAY_SIZE(button_masks); i++) {
		const uint32_t mask = button_masks[i];
		...
		if (button_states & mask) {
			button_state.pressed_buttons |= mask;
		...
		was_pressed = button_state.pressed_buttons & mask;
		button_state.pressed_buttons &= ~mask;

		if (mask == DK_BTN1_MSK) {
			/* Cancel long press timer if it was running and send short
			 * press
			 */
