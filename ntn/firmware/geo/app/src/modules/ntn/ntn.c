...
struct ntn_state_object {
	...
	bool is_registered;
	/* Holding the link open after a send, waiting for the next one. */
	bool lingering;
	/* Modem reported no suitable cell and is in limited service, searching
	 * periodically on its own.
	 */
	/* A valid position has been obtained since boot. Attaching without one
	 * would hand the modem latitude zero, longitude zero.
	 */
	bool have_fix;
	/* Button 1 was pressed with no fix: get one, then attach. One shot. */
	bool connect_after_fix;
};
...
static void gnss_location_work_fn(struct k_work *work)
{
	...
	/* One line per PVT on how the search is going, rather than one per
	 * satellite: enough to tell "no sky" from "almost there", and a tenth
	 * of the volume when the log is also going to flash.
	 */
	int tracked = 0;
	int in_fix = 0;
	int best_cn0 = 0;
	...
	for (int i = 0; i < NRF_MODEM_GNSS_MAX_SATELLITES; i++) {
		...
		best_cn0 = MAX(best_cn0, pvt_data.sv[i].cn0);
	}

	LOG_DBG("GNSS: %d SVs tracked, %d in fix, best C/N0 %d.%d",
		tracked, in_fix, best_cn0 / 10, best_cn0 % 10);
}
...
static void lte_lc_evt_handler(const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_NW_REG_STATUS:
		...
		// 	ntn_msg_publish(NTN_CELL_FOUND);
			...
			ntn_msg_publish(NTN_NO_SUITABLE_CELL);
...
static int set_ntn_active_mode(struct ntn_state_object *state)
{
...
#else
	/* The validation build attaches over a terrestrial cell through this
	 * same state machine. The system mode is the first of the handful of
	 * steps that differ; everything around them is shared, which is the
	 * point.
	 *
	 * Cat-M1, matching the terrestrial cellular profile exactly: once
	 * profiles are configured, the NTN modem firmware refuses to activate
	 * (+CME ERROR 514) unless the system mode matches a profile's access
	 * technology.
	 */
	err = lte_lc_system_mode_set(IS_ENABLED(CONFIG_APP_NTN_LINK_TN) ?
	...
	if (!IS_ENABLED(CONFIG_APP_NTN_MODEM_FW_TN)) {
		/* Satellite pointing. Inert over a terrestrial cell, but the
		 * call only exists on the NTN modem firmware.
		 */
	...
	if (!IS_ENABLED(CONFIG_APP_NTN_LINK_TN)) {
		/* SIB32 carries the satellite ephemeris. Requesting it is only
		 * valid in the NTN system mode; over a terrestrial cell the
		 * command is rejected whichever firmware the modem runs.
		 */
...
static int set_ntn_offline_mode(void)
{
	...
	/* Set modem to offline mode without loosing registration. Functional
	 * mode 45 only exists on the NTN modem firmware; the terrestrial one
	 * stops at 44, so on that firmware use plain offline.
	 */
	err = lte_lc_func_mode_set(IS_ENABLED(CONFIG_APP_NTN_MODEM_FW_TN) ?
...
{
	int err;
	int periodic_fix_retry = CONFIG_APP_NTN_GNSS_FIX_RETRY_SECONDS;
	enum lte_lc_func_mode mode;
...
{
	if (IS_ENABLED(CONFIG_APP_NTN_MODEM_FW_TN)) {
		/* Never enabled on the terrestrial firmware. */
		return 0;
	}
...
static int sock_send_gnss_data(struct ntn_state_object *state)
{
	...
	if (IS_ENABLED(CONFIG_APP_NTN_MODEM_FW_TN)) {
		/* SO_SENDCB only exists on the NTN modem firmware. On the
		 * terrestrial firmware the option is accepted and the callback
		 * never fires, so a send that succeeded sat waiting until the
		 * connection timer ran out and was reported as no cell.
		 *
		 * On that firmware do not wait for an acknowledgement at all: a
		 * send the modem accepted is treated as sent, and the state
		 * machine is told so the same way the callback would tell it.
		 */
...
static void try_send_gnss_data(struct ntn_state_object *state)
{
	...
	/* A send is in flight again: a timeout from here on is a failure, not
	 * an idle link being released.
	 */
...
static void state_running_entry(void *obj)
{
	...
	struct lte_lc_cellular_profile tn_profile = {
		.id = 1,
		/* LTE-M only. Upstream writes LTE_LC_ACT_LTEM || LTE_LC_ACT_NBIOT,
		 * a logical or that evaluates to 1, which is this. Spelled out so
		 * the intent and the effect agree: the validation link is Cat-M1,
		 * and the system mode must match this exactly or the NTN modem
		 * firmware refuses to activate (+CME ERROR 514).
		 */
		.act = LTE_LC_ACT_LTEM,
		.uicc = LTE_LC_UICC_PHYSICAL,
	};
	...
	if (!IS_ENABLED(CONFIG_APP_NTN_MODEM_FW_TN)) {
		/* %CELLULARPRFL is an NTN modem firmware command. On the
		 * terrestrial firmware it only produces an error at every boot.
		 */
...
/* The operator interface, as printed on the DK.
 *
 * Button 1 means one thing: try now. The kit attempts on its own at power-on
 * and retries on a timer when that fails; this skips the wait rather than
 * starting a different kind of attempt, so it does exactly what the retry timer
 * does. While an attempt is running, or the link is up, it does nothing - a
 * second press used to abandon an attach that takes minutes, and to drop a
 * working link.
 *
 * Button 2 sends the payload again over the link that is up. Button 3 stops
 * whatever is running and switches the radio off. Button 4 starts a new fix,
 * for when the kit has moved.
 */
static enum smf_state_result handle_button(struct ntn_state_object *state, uint8_t number)
{
	const struct smf_state *current = SMF_CTX(state)->current;

	switch (number) {
	case 1:
		if (current == &states[STATE_NTN]) {
			/* Including while the modem searches on its own after "no
			 * suitable cell": it retries every couple of seconds, so a
			 * search-now button would add nothing. Stop is button 3.
			 */
			LOG_INF("Button 1: %s, ignored",
				state->is_registered ? "already connected" : "already trying");

			return SMF_EVENT_HANDLED;
		}

		if (current == &states[STATE_GNSS]) {
			/* No position yet. Attaching without one points the modem
			 * at latitude zero.
			 */
			LOG_INF("Button 1: still searching for a fix, ignored");

			return SMF_EVENT_HANDLED;
		}

		if (!state->have_fix) {
			/* Try now means exactly that. Without a position the attach
			 * would carry latitude zero, so get the position first and
			 * attach the moment it arrives, rather than refuse and send
			 * the operator to another button.
			 */
			LOG_INF("Button 1: no fix yet, getting one first");
			state->connect_after_fix = true;
			smf_set_state(SMF_CTX(state), &states[STATE_GNSS]);

			return SMF_EVENT_HANDLED;
		}

		LOG_INF("Button 1: trying now");
		smf_set_state(SMF_CTX(state), &states[STATE_NTN]);

		return SMF_EVENT_HANDLED;

	case 2:
		if (current != &states[STATE_NTN] || !state->is_registered ||
		    state->sock_fd < 0) {
			/* Say so rather than drop it silently: a button that does
			 * nothing needs the log to explain why.
			 */
			LOG_WRN("Button 2: not connected, ignored");

			return SMF_EVENT_HANDLED;
		}

		LOG_INF("Button 2: sending");
		try_send_gnss_data(state);

		return SMF_EVENT_HANDLED;

	case 3:
		/* Stop whatever is running - a search, a held link, a fix - and
		 * switch the radio off. Distinct from a new fix on purpose:
		 * ending a test or saving the battery has nothing to do with
		 * having moved, and with the modem retrying on its own this is
		 * the only way to make it stop short of powering off.
		 */
		if (current == &states[STATE_IDLE]) {
			LOG_INF("Button 3: nothing running, ignored");

			return SMF_EVENT_HANDLED;
		}

		LOG_INF("Button 3: stop");
		state->connect_after_fix = false;
		ntn_led_error_clear();
	...
	default:
		LOG_DBG("Button %d has no function", number);
...
static enum smf_state_result state_running_run(void *obj)
{
	...
	if (state->chan == &NTN_CHAN) {
		...
		case IDLE_TRIGGER:
			...
			return SMF_EVENT_HANDLED;
		case NTN_TRIGGER:
			/* The backoff is rearmed by state_ntn_entry, for every
			 * path that enters NTN.
			 */
			smf_set_state(SMF_CTX(state), &states[STATE_NTN]);
		...
		const struct button_msg *btn = (const struct button_msg *)state->msg_buf;

		if (btn->type == BUTTON_PRESS_SHORT) {
			return handle_button(state, btn->button_number);
		}
...
static enum smf_state_result state_gnss_run(void *obj)
{
	...
	if (state->chan == &NTN_CHAN) {
		...
		case NTN_LOCATION_SEARCH_DONE:
			...
			/* An unattended tracker connects as soon as it can. An
			 * operator-driven build waits to be told, so park here
			 * instead and let the button start the attach.
			 */
			state->have_fix = true;

			if (IS_ENABLED(CONFIG_APP_NTN_AUTO_CONNECT) ||
			    state->connect_after_fix) {
				state->connect_after_fix = false;
				smf_set_state(SMF_CTX(state), &states[STATE_NTN]);
			} else {
				smf_set_state(SMF_CTX(state), &states[STATE_IDLE]);
		...
		case GNSS_TIMEOUT:
			...
			/* Usable but visibly not right: the buttons work, and the
			 * error LED says the position is missing. A pending attach
			 * is dropped: it would go out with no position.
			 */
			state->connect_after_fix = false;
			ntn_led_no_cell();
			smf_set_state(SMF_CTX(state), &states[STATE_IDLE]);
		...
		case NTN_TRIGGER:
			/* The retry backoff, or the operator, asking to attach.
			 * Not from here: there is no position yet, and attaching
			 * without one points the modem at latitude zero. Handled
			 * rather than propagated, because the parent would pull
			 * the state machine out of the search that is still
			 * running.
			 */
			LOG_DBG("NTN trigger while still searching for a fix, ignoring");
...
static void state_ntn_entry(void *obj)
{
	...
	*/
	...
	/* Rearm the retry backoff here rather than at each place that decides to
	 * come in. NTN is entered from three of them - the retry timer, the
	 * application, and the automatic path once a fix arrives - and only the
	 * first two used to rearm it. The automatic path did not, so the first
	 * backoff window was measured from boot instead of from the failure that
	 * started it. Doing it on entry makes "entering NTN restarts the
	 * backoff" true of every path.
	 */
...
static enum smf_state_result state_ntn_run(void *obj)
{
	...
	if (state->chan == &NTN_CHAN) {
		...
		case NTN_NETWORK_CONNECTION_TIMEOUT:
			...
			if (state->lingering) {
				/* Not a failure: the link was up and nobody used it.
				 * Drop it quietly, no error LED.
				 */
				LOG_INF("Link idle for %d s, releasing",
					CONFIG_APP_NTN_LINGER_SECONDS);
				smf_set_state(SMF_CTX(state), &states[STATE_IDLE]);
		...
		case NTN_NO_SUITABLE_CELL:
			/* The modem looked and found nothing. From here it searches
			 * periodically on its own, sleeping in between, on the
			 * schedule in %PERIODICSEARCHCONF.
			 *
			 * Say so on the error LED right away - the answer exists
			 * now, not at the end of the connection timer - but do not
			 * abort: the periodic searches are what catch the satellite
			 * coming into view, and cutting them would throw that away.
			 * With native search the connection timer is stopped here
			 * too, since the modem is now in charge of retrying.
			 */
			ntn_led_no_cell();

			if (IS_ENABLED(CONFIG_APP_NTN_NATIVE_SEARCH)) {
				k_timer_stop(&state->network_connection_timer);
				LOG_INF("No suitable cell; modem searching periodically, "
					"button 3 stops it");
			}
		...
		case NTN_TRIGGER:
			/* Already here. Propagating would let the parent re-enter
			 * this state, which restarts the attempt from scratch.
			 */
			return SMF_EVENT_HANDLED;
		...
		case NTN_SEND_ACK:
			...
			if (IS_ENABLED(CONFIG_APP_NTN_STAY_CONNECTED)) {
				/* Hold the link for the next send instead of going to
				 * sleep. A tracker wants to sleep; an operator with a
				 * send button wants the link to still be there when
				 * they press it. The same timer that bounded the
				 * attach now bounds the idle time on the link.
				 */
				state->lingering = true;
				k_timer_start(&state->network_connection_timer,
					      K_SECONDS(CONFIG_APP_NTN_LINGER_SECONDS),
					      K_NO_WAIT);

				LOG_INF("Payload acknowledged, holding the link for %d s",
					CONFIG_APP_NTN_LINGER_SECONDS);
...
static void state_idle_entry(void *obj)
{
	...
	/* Off means stopped; the heartbeat means a fix is in hand and button 1
	 * will connect. Nobody should have to guess which from a dark kit.
	 */
	ntn_led_idle(state->have_fix);
}
