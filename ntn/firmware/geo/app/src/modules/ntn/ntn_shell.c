...
#if defined(CONFIG_APP_NTN_BUTTON_INJECT_SHELL)
/* Button injection for validation.
 *
 * The operator interface is four physical buttons, which nothing automated can
 * press. This publishes the same message the button module publishes, so an
 * injected press takes exactly the path a real one takes rather than a test
 * shortcut that could pass while the real path is broken. It cannot, however,
 * catch a fault in the button module itself, which sits upstream of this.
 */
static int cmd_btn(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc != 2) {
		shell_print(sh, "Usage: att_ntn btn <1-4>");

		return -EINVAL;
	}

	number = strtol(argv[1], &end, 10);
	if (*end != 0 || number < 1 || number > 4) {
		shell_print(sh, "Button must be 1 to 4, got: %s", argv[1]);
	...
	struct button_msg msg = {
		...
		.button_number = (uint8_t)number,
	};

	err = zbus_chan_pub(&BUTTON_CHAN, &msg, K_SECONDS(1));
	if (err) {
		shell_print(sh, "Failed to inject button %ld, error: %d", number, err);

		return err;
	}

	shell_print(sh, "Injected button %ld", number);

	return 0;
}
#endif /* CONFIG_APP_NTN_BUTTON_INJECT_SHELL */

SHELL_STATIC_SUBCMD_SET_CREATE(sub_att_ntn,
#if defined(CONFIG_APP_NTN_BUTTON_INJECT_SHELL)
	SHELL_CMD_ARG(btn, NULL, "Inject a button press: att_ntn btn <1-4>", cmd_btn, 2, 0),
#endif
